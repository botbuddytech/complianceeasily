...
assert len(DEPTS)==36 and len(TRIGGERS)==10
...
manifest.update(data_release='1.1.0',base_research_as_of=ASOF,research_as_of=SCAN_DATE,indiafilings_additions=16,indiafilings_reviewed_topics=len(IF_SCAN))
(OUT/'validation_report.json').write_text(json.dumps(manifest,indent=2),encoding='utf-8')
(OUT/'schema.json').write_text(json.dumps({'schema_version':'1.0.0','import_order':order,'tables':{t:[{'name':c[0],'sql_type':c[1],'primary_key':i==0,'references':c[2] if len(c)>2 else None} for i,c in enumerate(cols)] for t,cols in SCHEMA.items()}},indent=2),encoding='utf-8')
